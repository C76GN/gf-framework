...
2. Run `python tools\gf_maintenance.py workspace-status --json` to classify current changes before choosing checks. In a dirty long-running workspace, pass repeated `--path <file>` arguments for the current turn's touched files before using the global recommendation list.

...
The legacy Asset Library submission should still represent the full GF addon package. Use the `gf-framework-7.0.0.zip` GitHub Release asset when the form accepts a package URL. If the legacy form only accepts a tag or commit, keep `Download Commit/URL` at `7.0.0`; `.gitattributes` keeps GitHub archive downloads focused on `addons/gf/**`.

Do not use `gf-kernel-7.0.0.zip` as the primary Asset Library download. The minimal kernel package is an advanced modular bootstrap path for users who intentionally want to install additional GF packages through `GF Package Manager`, the Godot-native package CLI, a release registry, or an offline bundle.
...
Enable the plugin to register the Gf AutoLoad and use the editor tools for extension management, GF module templates, typed accessors, and project constants. The official Asset Library package is the full `addons/gf` addon; advanced users can use the minimal kernel and modular packages from the GitHub Release when they intentionally want a package-managed bootstrap.
```
...
- Asset Version: `7.0.0`
- Download Commit/URL: `7.0.0`
- Preferred Package URL: `https://github.com/C76GN/gf-framework/releases/download/7.0.0/gf-framework-7.0.0.zip`
- Icon URL: `https://raw.githubusercontent.com/C76GN/gf-framework/7.0.0/addons/gf/icon.png`
...
2. Prefer the `gf-framework-7.0.0.zip` release asset URL if the legacy form accepts a direct package URL.
3. If the form only accepts a tag or commit, use `7.0.0` for `Download Commit/URL`.
4. Do not use `gf-kernel-7.0.0.zip` as the primary Asset Library package.
5. Use the icon raw URL with the same release tag.
6. Build the Asset Store package with `python tools\build_asset_store_package.py --version 7.0.0` and verify the zip root is `addons/`.
7. Run the GUT test suite on the target minimum Godot version.
8. Run `python tools\gf_maintenance.py release-status --version 7.0.0`.
9. Create the GitHub Release from a no-prefix SemVer tag such as `7.0.0`.

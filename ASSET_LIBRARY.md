...
The legacy Asset Library submission should still represent the full GF addon package. Use the `gf-framework-8.0.1.zip` GitHub Release asset when the form accepts a package URL. If the legacy form only accepts a tag or commit, keep `Download Commit/URL` at `8.0.1`; `.gitattributes` keeps GitHub archive downloads focused on `addons/gf/**`.

Do not use `gf-kernel-8.0.1.zip` as the primary Asset Library download. The minimal kernel package is an advanced modular bootstrap path for users who intentionally want to install additional GF packages through `GF Package Manager`, the Godot-native package CLI, a release registry, or an offline bundle.
...
- Asset Version: `8.0.1`
- Download Commit/URL: `8.0.1`
- Preferred Package URL: `https://github.com/C76GN/gf-framework/releases/download/8.0.1/gf-framework-8.0.1.zip`
- Icon URL: `https://raw.githubusercontent.com/C76GN/gf-framework/8.0.1/addons/gf/icon.png`
...
2. Prefer the `gf-framework-8.0.1.zip` release asset URL if the legacy form accepts a direct package URL.
3. If the form only accepts a tag or commit, use `8.0.1` for `Download Commit/URL`.
4. Do not use `gf-kernel-8.0.1.zip` as the primary Asset Library package.
5. Use the icon raw URL with the same release tag.
6. Build the Asset Store package with `python tools\build_asset_store_package.py --version 8.0.1` and verify the zip root is `addons/`.
7. Run the GUT test suite on the target minimum Godot version.
8. Run `python tools\gf_maintenance.py release-status --version 8.0.1`.
9. Create the GitHub Release from a no-prefix SemVer tag such as `8.0.1`.

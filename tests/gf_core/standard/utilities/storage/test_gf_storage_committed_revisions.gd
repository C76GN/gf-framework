...
func test_catalog_query_drains_schema2_writes_without_changing_read_revisions() -> void:
	assert_eq(_storage.create_revision_storage(), OK)
	assert_eq(_storage.save_data("catalog/a.json", {"value": 1}), OK)
	var old_token: String = _token("catalog/a.json")
	var rewrite: GFStorageAsyncOperation = _storage.save_data_request_async("catalog/a.json", {"value": 2})
	var addition: GFStorageAsyncOperation = _storage.save_data_request_async("catalog/b.json", {"value": 3})
	var limited: GFStorageCatalogResult = _storage.query_catalog("catalog", "json", true, {"max_file_count": 1})
	assert_true(rewrite.is_completed())
	assert_true(addition.is_completed())
	assert_true(rewrite.get_result().is_successful())
	assert_true(addition.get_result().is_successful())
	assert_true(limited.is_successful())
	assert_false(limited.is_complete())
	assert_eq(limited.get_files(), PackedStringArray(["catalog/a.json"]))
	var read: GFStorageReadResult = _storage.load_data("catalog/a.json")
	assert_true(read.ok)
	assert_eq(GFVariantData.get_option_int(read.payload, "value"), 2)
	var actual_token: String = read.get_committed_revision().get_revision()
	assert_false(actual_token.is_empty())
	assert_ne(actual_token, old_token)
	assert_eq(actual_token, _token("catalog/a.json"))
	var complete: GFStorageCatalogResult = _storage.query_catalog("catalog", "json", true, {"max_file_count": 2})
	assert_true(complete.is_successful())
	assert_true(complete.is_complete())
	assert_eq(complete.get_files(), PackedStringArray(["catalog/a.json", "catalog/b.json"]))
	assert_eq(_token("catalog/a.json"), actual_token, "只查询 catalog 不得产生新提交。")
	assert_eq(_storage.delete_file("catalog/b.json"), OK)
	var after_delete: GFStorageCatalogResult = _storage.query_catalog("catalog", "json", true)
	assert_true(after_delete.is_successful())
	assert_true(after_delete.is_complete())
	assert_eq(after_delete.get_files(), PackedStringArray(["catalog/a.json"]))
	assert_eq(_storage.query_committed_revision("catalog/b.json").get_status(), GFStorageRevisionResult.Status.NOT_FOUND)
...
func test_state_publish_failure_preserves_evidence_and_same_owner_query_recovers() -> void:
	...
	var commit_path: String = GFVariantData.get_option_string(descriptor, "transaction_commit_path")
	var commit_before: PackedByteArray = FileAccess.get_file_as_bytes(commit_path)
	var failed_catalog: GFStorageCatalogResult = _storage.query_catalog("outside", "json", true)
	assert_false(failed_catalog.is_successful(), "selector 外的待恢复提交也必须阻止全 root 查询成功。")
	assert_false(failed_catalog.is_complete())
	assert_true(failed_catalog.get_files().is_empty())
	assert_eq(failed_catalog.get_error_code(), ERR_CANT_CREATE)
	assert_eq(FileAccess.get_file_as_bytes(commit_path), commit_before)
	failing.fail_state_write = false
	var recovered_catalog: GFStorageCatalogResult = _storage.query_catalog()
	assert_true(recovered_catalog.is_successful())
	assert_true(recovered_catalog.is_complete())
	assert_eq(recovered_catalog.get_files(), PackedStringArray(["state.json"]))

...
- 新增 tracked-only `codeql_suppression_policy`：禁止 Python 源码中的全部内联 suppression，并拒绝 legacy、通配、多 query，以及 CodeQL 的路径过滤、查询过滤、自定义配置和自定义查询套件；该策略与 GF 凭据门禁的豁免保持完全隔离，并进入 quick、full 与 release。

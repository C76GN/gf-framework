...
- `GFWebSocketNetworkBackend` 的主机接入改为精确正容量：默认最多 32 个含握手中连接，`max_clients` / `max_peers` 只接受 `1..65_535` 的精确 int；每轮冻结并执行 accept、单 peer 包、全局包和服务 peer 四项不可关闭预算，先按公平游标服务现有 peer，再执行最多 16 次 take/accept/reject。入站/出站缓冲、排队包和 WebSocket 子协议也受公开绝对上限、精确类型及协议 token 校验约束；容量拒绝、握手失败、公平游标和预算用量进入调试快照，同步信号中的嵌套 `poll()` 为无副作用 no-op，不能重置当轮预算或重复派发生命周期信号。

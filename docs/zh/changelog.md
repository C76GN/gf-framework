...
91. AI Developer 项目先用 `contract-migration-plan` 审阅 v2→v3 计划，再以同一 `plan_sha256` 运行 `contract-migrate`；迁移只初始化 `architecture.path_roles: []`，需要角色时由项目明确补写并复核。直接消费项目 Snapshot v5 的工具必须按 `schema_version` 分流，升级到工具协议 `6.0.0` 后重新生成 v6，接受 `partial` 状态、新角色/advisory/越权字段和新的 edge kind 闭合集合；禁止原地迁移旧 Snapshot，或继续把任意 `res://` 字符串当成架构依赖。

...
- 修复类型化场景请求的四个终端边界：非主线程配置失败不再返回无法终结的半配置 Operation 或派发 Broker Lease；preload 终态 progress 发布期间的同路径重入以 busy 失败关闭，不再递归轮询旧 aggregate；同步自定义切场的显式确认与同步 `scene_changed` 只记录 current-generation 回执，并在 override 返回 true 及 owner/token 复核后结算，legacy true/no-hook override 仍保留匹配目标时的同步完成；已提交的运行时打包 `PackedScene` 不再因空 `scene_file_path` 被误报为切场失败，同时要求 committed root 身份不同于切换前 current scene，避免 no-op override 误收已有空路径 root。
...
- `GFSceneUtility.load_scene_request_async()` 与 `preload_scene_request_async()` 在主线程返回新的 `GFSceneOperation`；非主线程或 Operation 配置失败返回 `null` 且不派发，`progressed` / `completed`、request ID、kind、资源身份、进度、结果和 `cancel()` 都属于逐 consumer 契约。`GFSceneOperationResult` 公开 `COMPLETED`、`REJECTED`、`FAILED`、`CANCELLED`、`DISPOSED` 闭合状态及精确 `REASON_*` / `Error` 配对；`to_dict()` 冻结 exact 9-key schema：`status`、`successful`、`request_id`、`kind`、`scene_identity`、`scene`、`has_scene`、`reason`、`error_code`。protected `_confirm_target_scene_commit()` 供同步自定义 `_do_change_scene()` 提交 current-generation 回执；只在 override 返回 true 且 owner/token 仍有效后结算。旧 `load_scene_async()`、`preload_scene()`、全局场景信号与 path-level cancel 签名保持兼容。

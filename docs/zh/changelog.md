...
- `GFSceneUtility.load_scene_request_async()` 与 `preload_scene_request_async()` 返回新的 `GFSceneOperation`；`progressed` / `completed`、request ID、kind、资源身份、进度、结果和 `cancel()` 都属于逐 consumer 契约。`GFSceneOperationResult` 公开 `COMPLETED`、`REJECTED`、`FAILED`、`CANCELLED`、`DISPOSED` 闭合状态及精确 `REASON_*` / `Error` 配对；`to_dict()` 冻结 exact 9-key schema：`status`、`successful`、`request_id`、`kind`、`scene_identity`、`scene`、`has_scene`、`reason`、`error_code`。旧 `load_scene_async()`、`preload_scene()`、全局场景信号与 path-level cancel 签名保持兼容。

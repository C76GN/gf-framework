...
- AI Developer 新增闭合、opt-in 的 `architecture.documentation_roots` 与项目文档 API 新鲜度分析：只把 fenced Markdown code 与同一物理行内闭合、未转义的 inline code 中精确 `GF...` / `Gf` owner 或 `Owner.member` 形状作为高置信证据，并解析 catalog 可证明的 public/protected GF 继承成员；未知 owner/member actionable，普通 prose 与跨行/未闭合 inline 只 advisory。扫描仅保留 canonical 路径、行列、token 和 catalog 分类，不复制项目正文；根或 generated ownership 重叠、任意路径组件链接、strict UTF-8、目录项/文件数/单文件/总字节预算、文件/目录身份或 exact API catalog/framework/source digest/inheritance 不完整时统一拒绝 clean。

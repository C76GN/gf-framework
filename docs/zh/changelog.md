...
- AI Developer 项目契约新增闭合 `architecture.path_roles`：`scan_root`、`test_fixture` 与 `optional_input` 在不放宽高置信 load/resource 证据的前提下解释显式扫描根、测试夹具和可缺失输入。依赖分析新增有界角色、advisory 与 scan-root 越权证据，并以独立 `shader_include` edge kind 保留精确 Shader `#include` 依赖；扫描根覆盖的 Module/Adapter 必须属于引用来源自身或其 `allowed_dependencies`，否则以 `undeclared_scan_root_dependency` 阻断 clean，但不会伪造成依赖边。
- AI Developer 新增闭合 `architecture.source_domains` 与公开 API package-policy 分析：按最深 canonical 根区分 runtime/test/tool/editor，把精确 class/AutoLoad owner 映射到 API catalog package，并仅用 required∪optional 传递闭包授权；forbidden 在所有域永远 actionable，动态 GF 字符串只 advisory。扫描遵守安全 `.gdignore`、排除精确 `addons/gf` 而保留项目其他 addons，并让 catalog、UTF-8、预算、域根或身份不完整统一阻断 clean。
- AI Developer 新增闭合、opt-in 的 `architecture.documentation_roots` 与项目文档 API 新鲜度分析：只把 fenced Markdown code（含显式 blockquote/list container）与同一物理行内闭合、未转义的 inline code 中精确 `GF...` / `Gf` owner 或 `Owner.member` 形状作为高置信证据，并解析 catalog 可证明的 public/protected GF 继承成员与 class owner 内建 `new` 构造器（AutoLoad 不放宽）；container 结束会关闭其中未闭合的 fence，HTML comment 内禁用内容不产生证据，未知 owner/member actionable，普通 prose 与跨行/未闭合 inline 只 advisory。扫描仅保留可严格 UTF-8 编码的 canonical 路径、行列、token 和 catalog 分类，不复制项目正文；契约 JSON lone surrogate、POSIX surrogateescaped 路径、根或 generated ownership 重叠、任意路径组件链接、strict UTF-8、目录项/文件数/单文件/总字节预算、文件/目录身份或 exact API catalog/framework/source digest/inheritance 不完整时统一拒绝 clean。
...
- 新增共享的 `GFSceneOperation` 与 `GFSceneOperationResult`；Scene Utility 的 load/preload request API 为每个 consumer 冻结独立身份、进度、取消和闭合终态。同一路径 preload consumer 可共享 Broker 物理加载而独立结算；并发 load 使用 busy rejection，不做 replacement 或 `SUPERSEDED`；成功 load 只在安全帧场景切换完成后结算。
...
- **破坏性变更**：Combat Emitter 现在只解析 typed Definition，并以“全批 allocate/place/bind/reserve → deferred policy commit → 无回调 consume 全部 Session → 稳定发布 started/emitted → 释放 finished/retirement 屏障”的两阶段事务发射。任何 Session ACTIVE 前失败会精确补偿 cooldown/charge 并恰好一次退休 fresh/pool 候选；ACTIVE 后失败保留收费并以稳定 EndReason 收敛。`gf.combat` 的 `extension_version` 因此从 `2.0.0` 提升到 `3.0.0`。
- **破坏性变更**：AI Developer 工具协议升级到 `8.0.0`，项目 Contract schema 升级到 v5，项目 Snapshot schema 升级到 v8。v4→v5 只初始化 `architecture.documentation_roots: []`；空列表保留 opt-in 未启用且不声称文档 clean。Snapshot 新增闭合 `documentation_reference_analysis`，消费方必须重新生成 v8，不能迁移或手工补写 v7；旧 v1-v3 契约会先按既有步骤链式迁移，legacy schema 预置未来字段继续失败关闭。

- **破坏性变更**：AI Developer 工具协议升级到 `7.0.0`，项目 Contract schema 升级到 v4，项目 Snapshot schema 升级到 v7。v3→v4 只初始化 `architecture.source_domains: []`，但空列表会把全部未匹配脚本 fail-safe 归入 runtime；旧 tests 路径/`test_` 文件名启发式不再权威。Snapshot 新增闭合 `api_package_policy_analysis`，`gf_api_usage` / `test_gf_api_usage` 只作为 runtime/test class 投影保留，消费者必须重新生成 Snapshot，不能迁移或手工补写 v6。
...
- `GFStorageAsyncOperation` 拆成 caller 与物理两条 exactly-once 终态轴；旧 `completed/is_completed/get_result` 保持物理语义。排队请求在执行器接纳前可真实取消，已接纳 load 的 caller 取消为 `CANCELLED`，已接纳 save/delete 为 `OUTCOME_UNKNOWN`；活动执行记录、family 锁与 settling ownership 保留到物理终态写入，线程模式还会保留线程槽到真实线程退出。调度按 physical-first 与首个 caller 原因获胜；`dispose()` 真实取消尚未接纳的排队请求并同步收敛已接纳工作，不提前释放 Utility 状态。
...
- Combat 的目标排序改为一次性装饰候选快照，Projectile Catalog 对重复导出 ID 采用首个有效条目权威并在写入/移除时规范化；Gauge 动作结果固定为本动作自身的提交快照，属性集合清理则先脱离旧状态再发送通知。
...
- 保存 Provider 从后续 `tick()` 开始按全局预算公平轮转，软时间预算只阻止启动下一个 slice，不伪装成可抢占执行。准备完成后文档逻辑 move 到 `GFStoragePayloadTransfer`；Storage 执行器在本次新写入的编码与 temp、marker、final 事务提交前执行有界纯 Variant 图预检和物化，既有事务 recovery 与目录初始化仍是独立前置生命周期。Save 终态不再保留完整文档副本，完整文档只由 load 结果返回。
...
- Storage 执行器载荷预检现在同时限制 128 层深度、1,000,000 个值和 64 MiB 估算原始字节，拒绝 Object/Script typed container 元数据；诊断只保留结构索引、类型与预算计数，Save 通过隔离 Adapter 映射 section，不输出 key/value 或 key 派生摘要。
...
- 加固 AI Developer 的路径角色、模块依赖与 API package-policy 扫描：相对 Shader include 与 raw `load` / `preload` 恢复为高置信边，scan root 也覆盖尚未生成的 declared generated root，普通角色字符串只按原始 canonical 字面量匹配；全部角色声明共享一次调用的条目/字节预算，枚举项产生时即消费共享预算，根和后代目录在遍历期间固定身份。依赖分析不再跨全项目保留完整源码 token，并把收集时的文件身份/大小贯穿首读与二读，拒绝以旧预算分析替换或增长后的内容。API policy 现在让每次读取尝试按读取前固定大小预留累计字节、以同一大小限制 reader，失败或非法 UTF-8 都不回滚预算；同时在扫描期分别限制普通/actionable/advisory 证据、在读取前记录 test 域，并以独立目录项预算和脚本末次身份复核阻断不完整 clean。API catalog 同时拒绝缺失或非 `public` 的 owner visibility。
- 修复 AI Developer API policy 在安全 `.gdignore` 剪枝前先枚举并计费后代、使应忽略目录耗尽全局 entry budget，以及把与源码无关的 FIFO/socket/device 误记为不安全目录的问题；`.gd` 形状的特殊条目、link/reparse 与 metadata 失败仍保持 fail closed。模块依赖分析继续在首遍固定全部支持文件身份，但不再为 class discovery 重复词法分析资源与 Shader；GDScript raw `load` / `preload` 现在保留反斜杠路径分隔符，普通字符串转义和资源/Shader 词法语义保持不变。Shader exact `#include` 行前缀改为单遍有穷状态，不再对长单行中的每个字符串重复回扫与复制整行前缀。
- 修复 `GFProjectileCatalog` 在同 ID 的首条 entry 缺少 Definition 时过早返回 null、从而遮蔽后续有效 Definition 并让 Emitter 误报缺失的问题；get/has/ID 枚举、Emitter 解析与 prune 现在统一采用首个有效条目，set/remove 继续规范或移除该 ID 的全部重复条目。
...
- 移除 Projectile 的 HitBox/空间节点继承、`launch(Dictionary)`、`projectile_scene`、`default_context`、`auto_launch_on_ready`、`queue_free_on_finish`、共享 context 私钥以及 Catalog 的 scene 读写入口；不保留兼容壳。Projectile Runtime 现在是完整实例树中的普通 `Node`，命中由 Definition 显式绑定的同维 `GFHitBox2D | GFHitScan2D` 或 `GFHitBox3D | GFHitScan3D` source 提供，完整 root 的 fresh/pool retirement 由 Emitter 唯一拥有。
...
- Projectile Emitter 入口收紧为 `emit_projectile(launch_input, projectile_id) -> Node` 与 `emit_projectiles(launch_input, projectile_id, emit_count) -> Array[Node]`，并以 `projectile_emitted(projectile_root, session, launch_input)` 暴露 ACTIVE Session。Catalog 改为 `set_definition()` / `get_definition()` / `has_definition()` / `remove_definition()`；SpawnPattern 改为接收 typed LaunchInput。`GFProjectileMotion` 只创建私有 state 与计算 typed intent，唯一宿主写入边界移至 `GFProjectileBodyAdapter2D` / `GFProjectileBodyAdapter3D`。
- 新增 `GFBackgroundWorkContext`、`GFBackgroundWorkTask.get_cancellation_context()`，以及 `submit_cpu_work()` / `submit_io_work()` options 中的 `pass_cancellation_context`。启用时 worker 必须精确接收 `(input_data, context)`；关闭时继续使用既有 `(input_data)` 调用形状。Context 只公开线程安全的 work identity、取消状态、稳定 `CancellationReason` 与纯数据诊断快照，不会进入或放宽 input payload。
...
- AI Developer 工具协议升级到 `6.0.0`，项目 Contract schema 升级到 v3，项目 Snapshot schema 升级到 v6。Contract 的 `architecture` 新增必需闭合数组 `path_roles`；Snapshot 的 `module_dependency_analysis` 新增角色、advisory 与 scan-root 越权的计数、截断状态和有界证据，并把不完整状态统一为 `partial`。依赖 edge kind 的闭合集合现在是 `class_name`、`resource_load`、`resource_field`、`shader_include` 与 `generated_output`：裸 `load` / `preload`、已知 `ResourceLoader` 调用、Godot `ext_resource.path` 和精确 Shader `#include` 形成对应资源边，其他普通 `res://` 字符串不再以宽泛 `resource_path` 进入依赖闭包。
...
- `GFStorageUtility.AsyncExecutionMode` 是新的公开枚举，稳定值为 `AUTOMATIC = 0`、`THREADED = 1`、`COOPERATIVE = 2`；新的公开属性 `async_execution_mode` 默认 `AUTOMATIC`，并在首个合法异步请求入队后冻结。`max_async_thread_count` 只控制 `THREADED` 的并行度，`COOPERATIVE` 固定每个 lifecycle tick 最多执行一个完整任务；`worker_accepted` 兼容字段现在表示所选执行器已经接纳请求，不保证存在 Godot `Thread`。显式 `THREADED` 在无线程运行时让 `begin_activation()` 以 `ERR_CANT_CREATE` 失败；Save Profile 未新增执行模式 API，但无线程平台的 activation 与存取行为已修复，`gf.save` 的 `extension_version` 因此从 `6.2.0` 提升到 `6.2.1`；框架版本保持不变。
...
- `GFSceneUtility.load_scene_request_async()` 与 `preload_scene_request_async()` 在主线程返回新的 `GFSceneOperation`；非主线程或 Operation 配置失败返回 `null` 且不派发，`progressed` / `completed`、request ID、kind、资源身份、进度、结果和 `cancel()` 都属于逐 consumer 契约。`GFSceneOperationResult` 公开 `COMPLETED`、`REJECTED`、`FAILED`、`CANCELLED`、`DISPOSED` 闭合状态及精确 `REASON_*` / `Error` 配对；`to_dict()` 冻结 exact 9-key schema：`status`、`successful`、`request_id`、`kind`、`scene_identity`、`scene`、`has_scene`、`reason`、`error_code`。protected `_defer_target_scene_commit()` 供同路径异步 `_do_change_scene()` 声明 current-generation wait；`_confirm_target_scene_commit()` 在同步或异步 custom commit 安装 root 后冻结精确回执，pathless custom commit 必须使用它。两者都只在当前 generation 生效，最终结算仍要求 override 返回 true 且 owner/token 有效。旧 `load_scene_async()`、`preload_scene()`、全局场景信号与 path-level cancel 签名保持兼容。
...
91. AI Developer 项目先用 `contract-migration-plan` 审阅 v3→v4 计划，再以同一 `plan_sha256` 运行 `contract-migrate`；旧 v1/v2 会链式初始化 v3 的 `path_roles` 后再初始化 v4 的 `source_domains`。迁移后必须显式声明真实 test/tool/editor 根及必要的嵌套 runtime reset，不能继续依赖目录/文件名启发式。直接消费项目 Snapshot v6 的工具必须按 `schema_version` 分流，升级到工具协议 `7.0.0` 后重新生成 v7，接受四域 API package-policy、独立 actionable/advisory 计数和 runtime/test class-only 兼容投影；禁止原地迁移旧 Snapshot，或从完整 vendoring、lockfile 存在或其他域观测推断 package 授权。
92. 既有 Scene 调用无需强制迁移：只关心同步发起错误和全局路径信号时可继续使用 `load_scene_async()` / `preload_scene()`。需要逐请求关联、独立取消、owner/token 生命周期或安全帧提交结果时，改用主线程限定的 `load_scene_request_async()` / `preload_scene_request_async()` 并保存 Operation；request 返回后先处理 `null`，再检查 `is_completed()`，只在 pending 时连接 `completed`。并发 load 会以 `REASON_LOAD_BUSY` 拒绝而不会取代当前请求；取消一个 preload Operation 只释放它的 consumer Lease，只有旧 path-level cancel 才取消同路径全部 consumer。不要把 Broker 迟到完成解释为 caller 成功，也不要用全局场景信号关联单次 Operation。
93. 迁移 Combat Projectile：把旧 Projectile 节点改为完整 2D/3D root 下唯一的 `GFProjectile2D` / `GFProjectile3D` runtime，为场景建立 typed Definition，并显式配置 runtime path、0..N 同维 HitBox/HitScan typed union impact source paths、Motion、Lifetime 与同维度 BodyAdapter。把 Dictionary target/metadata 改为 `GFProjectileLaunchInput2D` / `GFProjectileLaunchInput3D`，把直接位置写入改为 MotionState → MotionIntent → BodyAdapter，把 Catalog scene API 改为 Definition 四件套。Emitter 管理的 root 不得再在 `projectile_finished` 中重复 free/release；直接调用 runtime `launch(binding, input)` 时则仍由项目自行退休完整 root。若 `emit_projectiles()` 在发布期 Emitter release 后返回空数组，不要推断 cooldown/charge 未提交，应以 Session/失败信号终态对账。
94. AI Developer 项目先用 `contract-migration-plan` 审阅 v4→v5 计划，再以同一 `plan_sha256` 运行 `contract-migrate`；旧 v1-v3 会先链式执行既有迁移。应用后只把需要校验 GF API 新鲜度的项目 Markdown 目录写入 `architecture.documentation_roots`，不要声明 `res://`、`res://addons/gf`、generated output、扫描排除目录或重叠根。空列表表示未启用而非 clean。直接消费 Snapshot v7 的工具升级到协议 `8.0.0` 后必须重新生成 v8，接受 `not_configured`、精确 catalog/framework/source identity、fenced/same-line inline high-confidence、prose advisory、独立 actionable 证据与 partial 状态；禁止迁移旧 Snapshot、复制项目正文到证据或把不完整扫描解释为没有 stale reference。

...
对已经读取并准备修改的用户文件，应把读取内容的 SHA-256 作为 `expected_previous_sha256` 一并提交。保存前目标内容发生变化时会返回结构化冲突并拒绝覆盖；`GFScriptPatchUtility` 已默认使用这一约束。该比较能阻断陈旧快照覆盖，但不替代需要显式恢复句柄和完整故障事务的多文件写入。

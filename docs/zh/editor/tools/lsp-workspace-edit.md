...
计划在进入底层文件事务前保持可重试：如果提交快照过时或磁盘来源漂移，提交失败且计划不被消费。所有复核通过后，计划先被一次性认领，再把计划中的来源 SHA-256 作为 `preflight_existing_sha256`、结果 SHA-256 作为 `expected_sha256` 交给 `GFArtifactWriteTransaction`。底层事务会在预检和每个可观察替换边界再次比较旧内容；无论底层事务成功还是失败，同一计划都不能再次提交。

这条 freshness 链只会拒绝已经能够观察到的磁盘漂移，不是跨进程 compare-and-exchange。Godot 的 GDScript 文件 API 无法把最后一次摘要读取与路径替换合并成不可分割操作；外部编辑器或进程若可能同时保存同一文件，宿主必须先建立平台原生锁、版本化存储或等价协调，再调用 Adapter。GF 不会用仅对自身调用方生效的 sidecar 锁声称消除了该窗口。
...
GDScript 文件 API 无法固定父目录 handle。适配器会在计划和提交阶段拒绝可观察到的链接，以精确规范宿主路径复核工作区 containment，并把来源/结果摘要作为底层可观察 freshness 复核输入；底层事务也会在替换前复核可观察路径、旧内容与 sidecar 状态。但本机恶意进程仍可能在这些路径调用之间交换目录或文件，因此该机制适用于受信工作区和协作式并发，不承诺封死敌对目录 TOCTOU。多文件提交提供运行期补偿，不承诺断电、进程终止或文件系统故障下的 crash atomicity。

...
恢复先校验快照结构、资源身份和目标文本行，再调用传入上下文的 `deserialize_values()` 覆盖。子类应先完整校验输入，再一次性替换 `values` 与自有持久状态；返回 `false` 必须保留这些状态，返回 `true` 表示全部恢复完成。此步骤是同步数据恢复，不应重新进入或推进 Runner；框架不会反射复制或回滚项目对象。

...
当前父队列会强持有每个已创建名称，绑定节点离树只清空对应子队列，不会从父队列的命名表移除；单个名称也没有独立 release 入口。因而当前契约下，名称应使用少量稳定通道，不应直接使用实体、请求或临时节点的无界动态 ID。`clear_all_named_queues()` 的 `stop_current` 是兼容占位参数，当前总是停止并释放全部子队列。

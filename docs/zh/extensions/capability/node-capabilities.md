...
Inspector 的“校验”按钮会检查当前节点能力的重复脚本，以及 `required_capabilities` 的缺失和歧义；当同一基类依赖匹配到多个不同子类脚本时会报告 `ambiguous_required_capability`，不会按第一个子类放行。编辑器校验只读取场景结构与导出属性，不会执行非 `@tool` 的项目能力脚本方法；这避免业务逻辑在编辑器中运行。它只辅助编辑器排查节点能力组合，不会自动补齐业务能力，也不会替代运行时 `GFCapabilityUtility.inspect_receiver()`。编辑器菜单也提供 `工具 > GF > 生成 Capability`、`生成 NodeCapability`、`生成 Node2DCapability`、`生成 Node3DCapability` 与 `生成 ControlCapability` 模板入口；这些模板生成的是项目类，因此不再冒用 GF Framework `3.17.0` 的版本来源，项目发布时应按自己的版本策略补充 provenance。

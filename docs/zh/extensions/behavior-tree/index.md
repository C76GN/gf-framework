...
`reset()` 是运行态重置通知，不是通用异步取消确认；内置 `Action` 也没有外部任务 owner/cancel 协议。项目若在节点外启动网络、导航、Timer、线程或信号等待，必须由项目自定义节点在自己的 `reset()` 中终止/隔离旧代际，或由外部系统明确持有其生命周期。自定义重写必须调用 `super.reset()`。回调中重置正在运行的树，或通过 `Decorator.set_child()` 更换子节点，会使旧 tick 返回 `ABORTED`；后续 tick 使用新状态。`set_child()` 先提交新子节点，再 reset 被替换的旧 child，避免旧清理回调覆盖新的拓扑；这个同步通知不表示外部工作已经取消完成。

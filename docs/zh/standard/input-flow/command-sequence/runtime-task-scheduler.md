...
任务进入调度器后不能再修改 requirement；需要变更占用对象时，应取消任务并重新调度。任务组的占用包括显式添加到组的 requirement 和子任务当前 requirement；重建子树聚合不会删除组自身声明的占用。并行任务组会拒绝包含重复 requirement 的子任务，避免组内绕过同一对象的互斥规则。

...
当前状态和暂停栈中的实例保持唯一；再次 push 当前或已经暂停的状态会被拒绝。
...
如果叠加状态或暂停栈状态在 `_exit()` 中请求切换到其他状态，状态组会按最后一次退出重定向进入目标状态。`pop_state()` 的退出重定向仍检查暂停状态的退出 guard；若 guard 拒绝，已经退出的叠加状态不会再次进入，而是完成原定 pop，恢复上一层状态。

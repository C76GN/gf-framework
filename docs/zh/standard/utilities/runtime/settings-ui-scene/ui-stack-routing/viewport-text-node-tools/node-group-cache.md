...
缓存会监听 `SceneTree.node_added` 和 `SceneTree.node_removed`，节点进入或离开树时自动标记为脏。下一次 `get_nodes()` 会重建快照；缓存干净时读取会复用上一次结果，并在返回前剪掉已释放、离树、移出 group 或不再匹配类型过滤的旧节点。一次同步使用临时实例 ID 集合完成成员校验，工作量随 group 结果与缓存快照线性增长，不会对两份节点列表做成对扫描。
...
`GFNodeGroupCache` 不替代显式注册表。需要 owner/token 生命周期、优先级、候选打分或跨场景持久化时，应在项目层或对应 Utility 中组合它，而不是把业务注册规则塞进 group cache。使用 `GFObjectCandidateRegistry` 时，降低正数 `max_candidates` 会立即按注册顺序淘汰最旧记录，并把整次容量收敛作为一次 revision 与一次变更通知；重复写入同一上限不产生变化。

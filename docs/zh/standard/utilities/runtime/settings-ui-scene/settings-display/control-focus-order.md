...
焦点资格使用 Godot 的有效焦点模式，包含祖先 `focus_behavior_recursive` 的禁用以及子分支显式启用的覆盖；在场景树中的控件也会检查祖先可见性。

当 `step == 0` 且 `current` 位于传入数组中时，查询只返回仍有资格的 `current`；它失去资格时返回 `null`，不会移动到其他控件。`current` 为空或不在传入数组中时，仍按步长方向从可用顺序的首项或末项开始。

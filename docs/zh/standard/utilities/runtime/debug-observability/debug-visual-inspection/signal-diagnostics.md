...
运行时事件只追加到发射记录，不会在每次信号触发时重新构建完整连接树；场景连接结构只在结构刷新点重建，避免高频信号把编辑器 UI 放大成全树重绘。`CONNECT_ONE_SHOT` 自动断开后，探针会在下一次查询或监听前清理失配账本，使同一信号可以重新 watch。

节点树监听默认带深度和数量上限，避免误选整个大型场景树时把所有信号都连上。信号源会持有绑定到探针的 `Callable`；调用方结束观察时必须调用幂等的 `dispose()` 或 `unwatch_all()`，编辑器面板也遵守同一责任。不要在生产构建默认开启全场景探针；面对远程调试、玩家可见工具或包含敏感参数的信号时，应由项目层限制范围、脱敏和权限。

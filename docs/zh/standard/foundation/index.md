...
- `GFPlatformLocaleMap` 提供平台语言 key 到 Godot locale / fallback locale 的纯数据映射，适合 Steam、微信、主机平台或自建启动器 adapter 复用；批量导入和逐条设置都采用规范化复合键的 last-write-wins，具体平台映射表应由项目或外部 adapter 提供。

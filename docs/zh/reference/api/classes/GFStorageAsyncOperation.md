...
| 常量 | [`OPERATION_RESET`](#member-gfstorageasyncoperation-constants-operation_reset) | `const OPERATION_RESET: StringName = &"reset"` |
...
<a id="member-gfstorageasyncoperation-constants-operation_reset"></a>
...
返回：`OPERATION_SAVE`、`OPERATION_LOAD`、`OPERATION_DELETE` 或 `OPERATION_RESET`。
...
显式结束当前 consumer 对物理请求的观察。 返回 true 只表示 caller 终态已被 Utility 线性化，不保证物理 worker 已取消。 save/delete/reset 已接纳时可能进入 `OUTCOME_UNKNOWN`，物理终态仍通过 `completed` 到达。
...
返回：等待中的 transfer-backed save 请求返回句柄；终态及普通 save/load/delete/reset 请求返回 null。

...
触屏 Node2D 控件共享底座。 提供触点捕获、隐藏/离树/process mode 禁用释放、屏幕坐标到画布坐标转换和输入 handled 标记。 具体按钮、摇杆、滑条或项目自定义触屏控件仍负责自己的形状、输出和业务无关配置。 直接调用 set_process_input(false) 不会产生可拦截的 Node 禁用通知；调用方必须先 release()。

...
- 如果 `release-status` 的 API baseline 摘要报告 removed classes、removed members、signature changes 或 extends changes，应按破坏兼容版本处理；除非维护者明确确认并在 changelog 或发布说明中记录理由，不要用 minor/patch 版本发布这类变更。维护者明确批准后，发布诊断可使用 `release-status --allow-breaking-api`，但默认 gate 必须继续拦截未授权的破坏性 minor/patch 发布。

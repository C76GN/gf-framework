...
`gut-shard-plan` 是 GUT 分片 G0 阶段的显式观测入口，不属于任何维护 suite，也不得加入 Ready/main/release DAG。无附加参数时保持只读，只把受版本控制的分片 manifest 与当前 `tests/gf_core/**/test_*.gd` 清单做闭合校验；`--junit build/...xml` 只解析一次已经存在且位于受控 `build/` 树内的 JUnit。只有维护者显式传入 `--run-gut` 时，才按原顺序执行 `godot_import` 与权威 `gut` 各一次；该模式会写入 ignored 的 `.godot/` 导入状态、调用期托管日志，以及随机 invocation-owned 的 `build/gut-sharding/<run-id>/gut-authoritative.xml`，不会覆盖或删除已有 XML，并使用独立的至少 1200 秒 GUT 观测预算，`--timeout` 只能提高该下限。顶层维护 CLI 会按既有 log session 规则清理成功调用新写的日志，失败证据则保留。需要保存机器相关时长时使用 `--json-output build/gut-sharding/<name>.json`，这些观测文件保持 ignored，不得提交。G0 manifest 只声明确定性 inventory 覆盖和初始 lane，不代表时长平衡、受影响测试选择或通过证明；任何 manifest/JUnit 漂移都失败关闭。该入口的 skip、cache read、cache write 与 reuse 计数必须恒为 0，不能影响原 `gut`、`full`、CI 或 release 的执行集合、次数和退出语义。

...
- Ready PR 与 `main` push 必须运行 `python tools\gf_maintenance.py check --suite full` 等价检查。Ready/main 的 framework 检查拆成并行的 `framework-gut`、`framework-lsp` 与 `framework-static`，再与 `package-contract`、`package-editor`、`package-cli-local`、`package-cli-network`、`package-godot-ci` 和 Windows process supervision 汇总到冻结当前 BASE SHA 的非 required `GF full validation (<BASE_SHA>)` marker；当前 Full 运行再由名称稳定的 `GF merge gate` 验证该 marker。顶层 run name 必须在 workflow run 创建时冻结 mode、PR、HEAD 与 BASE，使新 Full intent 在下游 marker 尚未创建时也能取代旧 epoch。`maintenance-self-test` 必须锁定这些 shard 的并集与 `full` / `package-ci` 集合等价。此外，Ready/main 必须在 `windows-latest` 运行聚焦的 `windows-process-supervision` 自测及 stdin staging / startup boundary 单测，并作为 Full marker 依赖，持续验证原生 Job Object 清理。tag release workflow 必须把完整发布产物集构建一次并通过 manifest 固定，再用 `release-status --version <tag> --artifact-manifest <path>` 校验该产物集；检查 shard 将 package Godot shard 替换为 `package-godot-release`。framework、全部 package matrix shard、release metadata 与同一份不可变发布产物必须全部通过。
- workflow 外层 deadline 是 Full 等价与发布证据合同的一部分：Ready/main `framework-gut` 必须精确设为 60 分钟；tag release `release-framework-checks` 必须使用维护工具拥有的 4800 秒 suite 总 deadline，并把 GitHub job 精确设为 90 分钟；手动 `main` Full 保持 90 分钟。Ready/main 的 60 分钟上限必须严格高于当前 2820 秒 `framework-gut` 子闭包（600 秒 import + 360 秒 lifecycle + 1200 秒权威 GUT + 600 秒 warnings，再加 60 秒子进程 envelope）。release 的 4800 秒总 deadline 还覆盖已测静态检查余量，90 分钟外层再为 setup、teardown 与结构化超时清理保留 600 秒；这不表示 30 个串行 release check 可以在一次调用中分别耗尽各自上限。不得让 GitHub 先于维护工具中止 job，否则结构化超时与清理证据会被截断。
- 本地 `check --suite full` 默认把等价检查计划分配到彼此隔离的临时工作区，以 3 个 worker 有界并行；可显式使用 `--jobs 2` 至 `--jobs 6` 调整资源占用，`--jobs 1` 只作为复现次序相关问题或诊断资源争用的串行退路。runner 按稳定计划分批物化工作区，每批并发数不超过 `jobs`，但必须在全局 admission barrier 完成前保留全部计划工作区，并用各自最终 cwd/冻结环境建立全部 action command contract；任一后批解析失败时，Godot 隔离探针、进度 `started` 和所有 shard 派发都必须保持为零。屏障通过后才按稳定 batch 顺序运行、验收并立即逐批清理，因此 Full 峰值磁盘占用按完整计划而不是 `jobs` 个工作区预算。资源密集的 `framework-gut` 在所有本地并行度下必须单独占用一个 batch，且未过滤的权威 `gut` 检查使用与显式全量观测及 qualification control 一致的至少 1200 秒测量下限；singleton 负责消除资源争用，timeout floor 负责覆盖同一权威检查的正常波动，两者不得互相替代。suite deadline 必须覆盖源码捕获、artifact 构建、Godot 隔离探测和 shard 运行的完整链路。Windows runner 必须把 clone、staging、data/config/cache 和临时目录投影到本地短路径预算内，并只在通过绝对路径、link/reparse、源码边界与直接本地 fixed-volume 身份校验的根下创建带随机名且身份固定的受管子目录；自动选择失败时可用 `GF_MAINTENANCE_VALIDATION_TEMP_ROOT` 指定这一短路径父目录，UNC/device、映射盘与 SUBST/别名路径必须 fail closed。未跟踪普通文件必须用身份固定句柄分块捕获并逐块遵守 suite deadline，单文件最多 64 MiB、合计最多 256 MiB；未跟踪 symlink 则必须在 `readlink` 前后复核同一 `lstat` 身份，不能把旧 metadata 与新 target 混入同一指纹。隔离副本不能共享 `.godot`、LSP 工作区状态或可写日志根；并行启动前必须用真实 Godot 验证平台原生 data/config/cache 与 `user://` 全部位于 shard 私有根，self-contained 或不支持的布局必须 fail closed。每个 shard 必须由保留至验收结束的 POSIX process group 或 Windows Job Object 拥有，直接子进程无论正常退出、失败、超时或取消，完整后代树都必须在报告读取前清空。父进程还必须校验各 shard 的检查集合、结构化结果、源码指纹与失败日志归属。单次 suite 中 package smoke 所需 package archive、registry、source manifest 与 offline bundle 只构建并封存一次；共享 artifact set 必须同时绑定 workspace fingerprint 和文件 hash，各消费者只能使用经过再次校验的私有副本，且其报告必须精确匹配父进程的 manifest SHA-256 与 artifact count，不得改写共享字节，也不得跨源码 revision 缓存复用。`full` 必须包含 `ai_developer_kit`、`gdscript_warnings` 和 `gdscript_lsp_diagnostics`；Ready/main 的 `framework-lsp` 与 release framework 同样执行完整 LSP 硬门禁，error、warning、诊断超时、连接或传输失败都必须失败。除非维护者明确批准并记录原因，不要削弱 CI 或 release 行为测试、Godot reload warning、路径卫生、API 和文档闸门。
- 除显式写入的 `examples_sync_write` 外，维护套件必须在结束时重新计算工作区指纹；起止不一致即以 `workspace_snapshot_integrity` 失败，不能把混合 revision 的结果当成可信成功。调用期文本缓存只保留每个路径的当前 live identity，避免同一长任务持续积累历史版本；这项 live-view 约束不等同于不可变源码捕获，需要强一致输入的并行 Full 与 package 消费者仍必须使用已封存工作区。长驻 MCP 每个请求都必须清空源码 API cache，并以 1 MiB 行上限、严格 UTF-8/JSON、闭合 tool schema、有限数值和结构预算拒绝畸形输入；LSP 客户端必须使用单调绝对 deadline、有界 header/body、唯一 `Content-Length` 与严格 JSON object。机器 JSON 输出先拒绝非有限数值，再通过同目录临时文件原子替换。上述边界由 `python tests/gf_core/tools/test_gf_maintenance_execution.py` 以及 quick、framework、full、release 中的 `maintenance_execution_tests` 持续验证。
...
- `python tools\gf_maintenance.py package-boundary --json` 是 GF 模块化发行包 manifest 硬 gate；它扫描 `packages/**/*.json`，以 1 MiB、64 层和 16384 个 JSON 值的硬预算读取 UTF-8 manifest，拒绝无效 schema、非白名单字段、在任意层 `metadata` 中隐藏下载/checksum/installer/签名/依赖策略、缺失或循环依赖、违反 `kernel <- standard <- extensions` 与 tool 单向挂载规则的包依赖方向，以及多个包声明重叠源码路径。source manifest 的正向 `paths` 与 `exclude_paths` 在 builder 和维护归属审计中必须使用同一套严格、大小写敏感且平台无关的匹配语义；在 registry、lockfile 和原生 resolver 的排除契约完成前，不得把现有显式正向发行清单机械压缩成 broad root 加排除项。
- `python tools\gf_maintenance.py package-closure-audit --json` 是 GF 模块化安装闭包 report-only gate；它从 `packages/**/*.json` 计算每个 package/preset 的真实安装闭包、standard fan-in，以及源码 payload 的文件数、GDScript 数和未压缩字节数，使 package ID 不变时的 broad-root 载荷增长仍可观察。warning 记录过大的 extension 闭包、直接依赖完整 debug 包、debug 闭包拉入 UI 等边界债务，并 hard fail runtime extension 闭包包含 `gf.standard.editor` 的情况；在建立审阅基线前不得凭任意比例把 payload 指标直接升级成硬阈值。
- `python tools\gf_maintenance.py package-source-boundary --json` 是 GF 模块化发行包文件归属与源码引用硬 gate；它要求所有可发行的 `addons/gf` 文件由且仅由一个非 preset package manifest 拥有，并在归属、重叠和引用检查中一致应用所属 manifest 的 `exclude_paths`；显式忽略 Godot 生成的 `.import` sidecar，并扫描源码/配置文件，拒绝引用未被本包或直接依赖包拥有的 `addons/gf` 路径或 `class_name`。窄例外只有三类：根插件发现 standard 编辑器贡献、根插件同层 data-only catalog 声明已安装可选 `gf.tool.*` 编辑器贡献，以及内核扩展基础设施知道扩展根目录；这些例外不能扩散成 kernel/standard 对具体可选包的认识。
...
- `python tools\gf_maintenance.py package-focused-gut-mapping --json` 是 GF 模块化包到 focused GUT 归属关系硬 gate；它验证每个非 preset package 都在 `tests/gf_core/package_focused_gut_mapping.json` 中声明维护侧最小测试集合，测试路径存在且位于对应 kernel/standard/extension 测试范围内。当前 gate 只证明映射归属与测试形状，不执行 mapped tests，也不得被解释为测试已在 package 的最小安装闭包内通过；closure 行为证据必须由独立 executor 提供。该映射属于维护策略，不写入发行 package manifest。
...
- `check` 必须先验证检查 DAG，再按稳定拓扑序执行；共享依赖只执行一次，依赖失败时下游必须明确 blocked，不能继续制造噪声。维护边界的纯静态审计必须在同一进程执行，并在单次 suite 生命周期内共享 `tools/gf_workspace_snapshot.py` 的 UTF-8 文本、文件清单和 Git 路径快照；只有明确要求每次读取都固定完整父目录链、普通文件句柄并复核并发替换的安全 gate，可以复用同一 Git inventory 但绕过普通文本 cache 执行受控 pinned read。其他静态脚本可通过进程内 adapter 保留自己的专用解析器，不能因此重复启动 Python。快照不能跨 MCP 请求永久复用。源码标识符查询必须复用快照生成的 token set，package ownership 必须复用 `tools/gf_package_paths.py` 编译后的 matcher/index，保持平台无关和大小写敏感，并在 owner 索引层统一应用 source manifest 的 `exclude_paths`；manifest `paths` / `exclude_paths` 必须已经是 shared portable identity 接受的 canonical raw syntax，validator、matcher、compiled index 和 consumer 均不得先静默规范化再接受。builder 与维护审计不得各自维护匹配或排除语义。registry/resolver 继续消费有效正向 `paths`，除非未来通过版本化契约把排除语义完整传播到 registry、lockfile、Python resolver 和原生 backend。
...
- 发布前使用 `python tools\build_gf_release_artifacts.py --version <version> --output-dir build/release` 一次性生成 Asset Store ZIP、AI Developer Kit 独立 ZIP、release registry、registry source、offline bundle、全部非 preset package ZIP 和 `gf-release-artifacts-<version>.json`。同一次运行中每个 package archive 与 AI Developer Kit 都只能构建一次，online registry 与 offline bundle 必须复用同一批 archive 字节；manifest 必须记录源码 revision、角色、大小和 SHA-256。随后必须运行 `python tools\gf_maintenance.py check --suite release --artifact-manifest build/release/gf-release-artifacts-<version>.json`，让完整 framework、all-package matrix 与 `release-status` 共同消费该 manifest；不得以 `full` suite 或单独的 metadata check 代替 pre-tag release suite，也不得在检查或发布阶段重新构建一套“等价”产物。`--allow-dirty` 只能用于本地诊断，不能用于正式发布或 tag 前检查。

...
- 项目契约 schema v5 的 `architecture.path_roles` 只接受精确 `res://` 可移植身份与闭合集合 `scan_root`、`test_fixture`、`optional_input`，不得扩张为 glob、ignore 或任意 metadata：`scan_root` 只有在现存安全目录的全部有界后代都能证明归属时才完整，且每个引用来源只能覆盖自身或其 `allowed_dependencies` 在共享依赖命名空间中已声明的 Module/Adapter；兼容字段 `covered_modules` 必须按稳定顺序保留两类 owner ID。越权覆盖必须保留有界证据并阻断 clean，但不能伪装成 load/resource edge。`test_fixture` 只解释 test source domain 的普通字符串，`optional_input` 只解释精确文件且允许缺失；角色自身或后代出现 link/reparse、角色遍历预算截断，或依赖扫描文本出现非法 UTF-8、身份漂移时，分析必须保持 partial，不能产生 clean 结论。
...
- `api_package_policy_analysis` 只把注释外精确命中 API index public class/AutoLoad 的 identifier 映射到 exact `package_id`；允许集合只能来自 required∪optional 的传递 package 闭包，任何域的观测、完整 vendoring 或 lockfile 有无都不能扩张它，forbidden 必须优先且永远 actionable。known/unknown GF-shaped dynamic strings只可 advisory。普通 observation、actionable 与 advisory 必须独立计数和截断；catalog schema/version/source digest、未知/循环包图、public owner 缺 package、严格 UTF-8、文件/目录/marker 身份或预算不完整时不得输出 clean。

...
- 项目契约 schema v4 的 `architecture.path_roles` 只接受精确 `res://` 可移植身份与闭合集合 `scan_root`、`test_fixture`、`optional_input`，不得扩张为 glob、ignore 或任意 metadata：`scan_root` 只有在现存安全目录的全部有界后代都能证明归属时才完整，且每个引用来源只能覆盖自身或其 `allowed_dependencies` 在共享依赖命名空间中已声明的 Module/Adapter；兼容字段 `covered_modules` 必须按稳定顺序保留两类 owner ID。越权覆盖必须保留有界证据并阻断 clean，但不能伪装成 load/resource edge。`test_fixture` 只解释 test source domain 的普通字符串，`optional_input` 只解释精确文件且允许缺失；角色自身或后代出现 link/reparse、角色遍历预算截断，或依赖扫描文本出现非法 UTF-8、身份漂移时，分析必须保持 partial，不能产生 clean 结论。
- 项目快照 schema v7 的模块依赖分析只把裸 `load` / `preload`、已知 `ResourceLoader` 调用、Godot 文本资源 `path` 字段、注释外且同一行双引号前缀精确为 `#include` 的 Shader include、`class_name` 与显式声明 generated output ownership 作为高置信边。其他 `res://` 字符串只记录为 advisory、既有显式 `owned_resources` evidence 或闭合 path-role evidence，不得进入依赖闭包或单独改变 clean verdict；任何输入不完整状态统一输出 `partial` 并阻断 clean。

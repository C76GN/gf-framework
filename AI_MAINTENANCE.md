...
- `check --validation-eligibility-shadow` 只能在正常检查计划和结束工作区指纹验证完成后，对受限静态检查额外执行冻结快照观测；默认命令、parallel child、Ready/main/release 不得自动继承。该阶段仍固定 `execute`，不得改变 suite、DAG、原检查执行次数、结果或退出码，skip/cache read/cache write/persist/reuse 计数必须全部为 `0`。只有输入与工具实现从同一封存工作区读取、隔离 worker 的工具链和受控环境进入 Action Key、两次结构化结果一致、输出 schema/计数/质量信号闭合且执行后 manifest 未漂移，才可报告结构资格；任一捕获、materialize、worker、deadline、cleanup 或结果异常都只能 fail closed 为 ineligible。结构资格不是历史命中、可复用授权或合并证据；引入任何 Ledger 读写、CAS、跳过或调度行为必须另行审查。该内部 CLI、`validation_eligibility` 字段和实现模块不得泄漏到正式公开文档。

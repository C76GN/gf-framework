...
- 项目契约 `architecture.source_domains` 是 runtime/test/tool/editor 的唯一权威源码域声明：最深 canonical `res://` 路径段匹配优先，未匹配脚本 fail-safe 归入 runtime，旧 tests 目录或 `test_` 文件名启发式不得继续参与判定。声明不能位于大小写等价的 `addons/gf`、任意层级的 scanner-excluded segment 或 target-only generated root 内；缺失、非目录、link/reparse、`.gdignore` 不安全或输入身份漂移必须使 API policy 分析 partial。扫描仅保留 exact reserved `addons/gf` 排除，项目自有其他 addons 必须可观察。

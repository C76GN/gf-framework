...
- Release tag: `9.0.1`
- Minimum Godot version: `4.7`
- Primary download package: `gf-framework-9.0.1.zip`
- Primary download URL: `https://github.com/C76GN/gf-framework/releases/download/9.0.1/gf-framework-9.0.1.zip`
- Download package should use the attached `gf-framework-9.0.1.zip` release asset generated by `tools/build_asset_store_package.py`, not GitHub's automatic source archive and not the minimal `gf-kernel-9.0.1.zip` package.
...
- Advanced modular release assets, for GitHub Release users rather than the Store primary download:
  - `gf-kernel-9.0.1.zip`
  - `gf-registry-9.0.1.json`
  - `gf-registry-source.json`
  - `gf-package-offline-bundle-9.0.1.zip`
  - `gf-ai-developer-kit-9.0.1.zip`
  - package zips such as `gf-standard-base-9.0.1.zip` and `gf-extension-save-9.0.1.zip`
- Release metadata check: `python tools\gf_maintenance.py release-status --version 9.0.1`
...
Use these values for the `9.0.1` Store update:

1. Create or edit version `9.0.1`.
2. Set minimum Godot version to `4.7`.
3. Upload `gf-framework-9.0.1.zip` as the version download.
4. Do not upload `gf-kernel-9.0.1.zip` as the primary Store package.

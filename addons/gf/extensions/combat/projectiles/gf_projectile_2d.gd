...
var _impact_sources: Array[GFHitBox2D] = []
...
func _physics_process(delta: float) -> void:
	...
	var binding: GFProjectileBinding2D = _binding
	var motion: GFProjectileMotion = _active_motion
	var adapter: GFProjectileBodyAdapter2D = _active_adapter
...
func _connect_impact_sources(binding: GFProjectileBinding2D, generation: int) -> void:
	...
	for source_node: Node in binding.get_impact_sources():
		if not source_node is GFHitBox2D:
			continue
		var source: GFHitBox2D = source_node
...
func _disconnect_impact_sources() -> void:
	for index: int in range(mini(_impact_sources.size(), _impact_callbacks.size())):
		var source: GFHitBox2D = _impact_sources[index]

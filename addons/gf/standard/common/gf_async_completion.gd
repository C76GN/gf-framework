...
var _cancel_token: GFCancelToken = null
...
func cancel(reason: StringName = &"cancelled", metadata: Dictionary = {}) -> bool:
	return _complete(Status.CANCELLED, null, "", reason, metadata)
...
func bind_cancel_token(token: GFCancelToken) -> bool:
	...
	_cancel_callback = func(reason: StringName, metadata: Dictionary) -> void:
...
## @since 7.0.0
## [br]
## @param options: 等待选项，支持 GFAsyncWaitUtility.await_signal() 的选项。
## [br]
...
func wait_async(options: Dictionary = {}) -> Dictionary:
	...
	var wait_result: Dictionary = await GFAsyncWaitUtility.await_signal(completed, options)
...
func get_result() -> Variant:
	return GFVariantData.duplicate_variant(_result)
...
func get_debug_snapshot() -> Dictionary:
	...
	return {
		...
		"result": GFVariantData.duplicate_variant(_result),
		"error": _error,
...
func _complete(
	...
	_disconnect_cancel_token()
	_status = status
	_result = GFVariantData.duplicate_variant(result)
	_error = error
	...
	match _status:
		Status.SUCCEEDED:
			succeeded.emit(GFVariantData.duplicate_variant(_result), _metadata.duplicate(true))
		Status.FAILED:
...
func _disconnect_cancel_token() -> void:
	if _cancel_token != null and _cancel_callback.is_valid() and _cancel_token.cancelled.is_connected(_cancel_callback):
		_cancel_token.cancelled.disconnect(_cancel_callback)
	_cancel_token = null
